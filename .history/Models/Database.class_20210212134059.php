...
class Database{
	...
	public function et($id,$operateur) {
		$this->query .= " AND ".$id."".$operateur."?";

		return $this ;
	}
	public function ou($id,$operateur) {
	...
	Database::init("localhost","scolarite","3306","root","");

	//ALTER TABLE `note` ADD `francais` IF NOT EXISTS DOUBLE NOT NULL DEFAULT '0.0'
$db = new Database();
//  $tab = $db->select("note")
                     $tab = $db->select("etudiant")
                    ->innerD("note","id","id_etudiant")
					->getQuery();
?>
