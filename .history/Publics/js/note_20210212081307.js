function affiche_note_matiere() {
    $.ajax({
        ...
        success: function(data) {
            var button = "<a href='#' class='btn btn-primary col-sm-offset-5' id='btn_note'>Enregistrer</a> ";
            if (data.length > 0) {
...
$('#select_etudiant_note').on('change', function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            // if (data.length > 0) {
            //     var table_affichage = "<div'> <table class='table table-striped  m-t-10' style='position: relative !important;'><thead style='position: sticky !important;z-index: 1000;'><th>Nom de l'etudiant'</th><th>Français</th><th>English</th><th>Mathématiques</th></thead><tbody>";
            //     for (var i = 0; i < data.length; i++) {
            //         table_affichage = table_affichage + "<tr><td>" + data[i].nom + "</td><td>"+data[i].nom +"</td><td><a href='#' class=' glyph'  data-toggle='tooltip' data-placement='bottom' title='effacer' onclick='supr_mat(" + data[i].id_matiere + ")'> <i class='fa fa-trash'></i></a><a href='#' class='glyph' data-toggle='tooltip'  data-placement='bottom' title='modifier' onclick='modif_mat(" + data[i].id_matiere + ")'> <i class='fa fa-edit'></i></a>";
            //     }
