        ...
        success: function(data) {
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    table_affichage = table_affichage + "<tr><td>" + data[i].nom_matiere + "</td></tr><tr><td><input type='number' class='form-control' data-id='" + data[i].id_matiere + "' ";
                }
                table_affichage = table_affichage + "</td></tr></tbody></table></div>";

                $('.matiere_table').html(table_affichage);
            } else {
