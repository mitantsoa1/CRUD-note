...
function modif_mat(id) {
    $.ajax({
        url: "Controls/ajax.php",
        type: "POST",
        data: "id_mat_modifier=" + id,
        datatype: "json",
        success: function(data) {
            if (data != "") {
                $('#id').val(data[0].id_matiere);
                $('#matiere').val(data[0].nom_matiere);

                $('#btn_matiere').addClass('hide');
                $('#btn_matiere_modif').removeClass('hide');
                $('#btn_matiere_annuler').removeClass('hide');
            }

        }
    })
}
$('#btn_matiere_modif').on('click', function() {
    var $matiere = $('#matiere').val();
    var $id = $('#id').val();
    var data = "nom_mat_modif=" + $matiere + "&id=" + $id;
    $.ajax({
        url: "Controls/ajax.php",
        type: "POST",
        data: data,
        ...
        success: function(data) {
            if (data == "ok") {
                ...
                $('#btn_matiere').removeClass('hide');
                $('#btn_matiere_modif').addClass('hide');
                $('#btn_matiere_annuler').addClass('hide');
...
$('#btn_note_register').on('click', function() {
    alert('ici');
})