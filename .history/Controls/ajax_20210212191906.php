<?php 
    ...
    if(isset($_POST['id_et'])){
        ...
        $tab =$db->select("note")
            ...
            if(count($tab)>0){
                ...
                $db->insert("note")
                    ->parametters(["id","francais","anglais","mathematiques","Malagasy"])
    ...
    if(isset($_POST['id_mod_note'])){
        $tab = $db->select("note")
                    ->where("id_note","=")
                    ->execute([$_POST['id_mod_note']]);
        echo json_encode($tab);
    }
    if(isset($_POST['id_et_modif'])){
        $fr=htmlspecialchars(trim($_POST['fr'])) ;
        $ang =htmlspecialchars(trim($_POST['ang'])); 
        $maths =htmlspecialchars(trim( $_POST['maths'])) ;
        $mal =htmlspecialchars(trim($_POST['mal'])) ;
        $id_et = $_POST['id_et'];
        $id_n = htmlspecialchars(trim($_POST['id_n'])) ;

        
                $db->update("note")
                    ->parametters(["id","francais","anglais","mathematiques","Malagasy"])
                    ->where("id_note","=")
                    ->execute([$id_et,$fr,$ang,$maths,$mal,$id_n]);
                    echo json_encode("ok");
